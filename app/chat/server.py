# practically a copy of https://github.com/wesdoyle/python-twisted-chat-server/blob/master/chatserver.py 
...
class LineReceiver3(LineReceiver):
    def sendLine(self, line):
        super().sendLine(line.encode(errors='ignore'))

class ChatProtocol(LineReceiver3):
    ...
    def connectionLost(self, reason):
        ...
        chatLog.append(str(self.name) + " exits.")
    ...
    def handle_REGISTER(self, name):
       ...
       if len(self.factory.users) > 1:
           self.sendLine('Participants in chat: %s ' % (", ".join(map(str, self.factory.users))))
       else: 
           self.sendLine("You're the only one here, %r" % name)


    def handle_CHAT(self, message):
       message = self.getTime() + "<%s> %s" % (str(self.name), str(message))
    ...
    def updateSessionInfo(self):
        ...
        molding = "============================================"
        print(molding)
        print('Users in chat: %s ' % (", ".join(map(str, self.factory.users))))
        print(molding)
        global chatLog
        chatLog = chatLog[-20:]
        print("\n".join(chatLog))
